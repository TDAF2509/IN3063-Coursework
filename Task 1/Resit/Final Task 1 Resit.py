...
def aco_pathsolver(grid, h, w, num_ants=10, num_iterations=100, alpha=1, beta=1, evaporation_rate=0.1):
    verticesN = h * w
    pheromone_matrix = [[1.0] * verticesN for _ in range(verticesN)]
    ...
    for _ in range(num_iterations):
        ...
        for _ in range(num_ants):
            ant_path = []
            current_vertex = 0
            while current_vertex != verticesN - 1:
                neighbors = [i for i in range(verticesN) if i != current_vertex]
                probabilities = [pheromone_matrix[current_vertex][neighbor] ** alpha *
                                 (1.0 / abs(grid[current_vertex // w, current_vertex % w] - grid[neighbor // w, neighbor % w])) ** beta for neighbor in neighbors]
                total_probability = sum(probabilities)
                probabilities = [p / total_probability for p in probabilities]
                chosen_vertex = random.choices(neighbors, probabilities)[0]
                ant_path.append(chosen_vertex)
                current_vertex = chosen_vertex
            ant_paths.append(ant_path)
        
        for i in range(verticesN):
            for j in range(verticesN):
                pheromone_matrix[i][j] *= (1 - evaporation_rate)
        
        for ant_path in ant_paths:
            total_cost = sum([abs(grid[vertex // w, vertex % w] - grid[next_vertex // w, next_vertex % w])
                              for vertex, next_vertex in zip(ant_path, ant_path[1:])])
            for vertex, next_vertex in zip(ant_path, ant_path[1:]):
                pheromone_matrix[vertex][next_vertex] += 1.0 / total_cost
    
    best_path = min(ant_paths, key=lambda path: sum([abs(grid[vertex // w, vertex % w] - grid[next_vertex // w, next_vertex % w])
                                                      for vertex, next_vertex in zip(path, path[1:])]))
    
    total_cost = sum([abs(grid[vertex // w, vertex % w] - grid[next_vertex // w, next_vertex % w])
                      for vertex, next_vertex in zip(best_path, best_path[1:])])
    
    return total_cost, [(vertex // w, vertex % w) for vertex in best_path]
...
def gamemenu():
    ...
    print('8. Game mode (Ant Colony Optimization)')
    print('9. Quit game')
...
while True:
    ...
    # ACO mode
        ...
        creategrid()
        costAndPath = aco_pathsolver(grid, h, w, num_ants=20, num_iterations=100, alpha=1, beta=2, evaporation_rate=0.1)
        displaycostandpath(grid, h, w, costAndPath, "ACO")
        print("\n")
    # User quit game
    elif userChoice == '9':
